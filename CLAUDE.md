...
- **Вордстат / спрос** — `tools/keyword-research/`: `suggest.py` (реальные формулировки из подсказок Яндекса и Google, доступов не требует), `wordstat.py` (частотность; режим `--phrases` готовит фразы с операторами для ручной выгрузки, `--api` требует доступа и дозаполнения контракта), `bing_kw.py` (международный трек: точные объёмы из Bing Webmaster Tools бесплатно, ключ в `.secrets/bing-webmaster.env`), `score.py` (вердикт по кластерам). Доступ к частотности в рунете: API Вордстата (бета, по заявке в поддержку Директа) либо Wordstat в Yandex Cloud Search API. Креды — `.secrets/wordstat.env`. Метод и правила чтения — `pet-projects/demand-research.md`.
