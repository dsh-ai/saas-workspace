...
3. **Зеркало для сайта.** После правки статьи в `Контент/<slug>/` синхронизировать с `../landing/website_dev/content/<slug>/` (с сохранением поля `published` в frontmatter и нормализаций — нет строки «Целевая аудитория:», нет висячих `---` после H1). Без зеркала изменения не попадут на блог. **Важно:** `cp` целиком затирает landing-копию, включая `published`. После `cp` ВСЕГДА восстанавливать `published: "YYYY-MM-DD"` через Edit между `type: pillar` и закрывающим `---`.
...
| К4 | Философия и метрики РОПа | запущен 2026-05-11 (3 pillar) |
| К5 | Внимательная работа с опросниками | запущен 2026-05-17 (4 pillar, разворачивают пиллары главной 1:1) |
